server <- function(input, output) {
  ...
  selected_pcode <- reactiveVal("21UGA006003")
  ...
  output$selected_district <- renderText({
    paste("You have selected", uga_admin %>% filter(pcode == selected_pcode()) %>% pull(ADM1_EN))
  })
  ...
  output$rainfall_glofas_plot <- renderPlotly({
    if(has_glofas()){
    p <- plot_glofas(#rainfall(),
                     glofas(), impact_df(), #input$rainfall_threshold,
                     input$glofas_threshold, has_glofas())
    p
    }
  })
  ...
  result_table <- reactive({
    if(has_glofas()){
      predict_with_glofas_and_rainfall(all_days, #rainfall(),
                                                             glofas(), impact_df(), #input$rainfall_threshold,
                                                             input$glofas_threshold, has_glofas())
      }
    })

  floods_val <- reactive({if(has_glofas()){result_table() %>% pull(floods)}})
  floods_correct_val <- reactive({if(has_glofas()){result_table() %>% pull(floods_correct)}})
  floods_incorrect_val <- reactive({if(has_glofas()){result_table() %>% pull(floods_incorrect)}})
  protocol_triggered_val <- reactive({if(has_glofas()){result_table() %>% pull(protocol_triggered)}})
  detection_ratio_val <- reactive({if(has_glofas()){result_table() %>% pull(detection_ratio)}})
  fals_alarm_ratio_val <- reactive({if(has_glofas()){result_table() %>% pull(false_alarm_ratio)}})
  ...
  output$impact_map <- renderLeaflet({
    leaflet() %>%
      ...
      addPolygons(data = uga_admin, label=~ADM1_EN, layerId=~pcode, col=~flood_palette(n_floods), fillOpacity=0.8, opacity = 1, weight=1.2)
  })
