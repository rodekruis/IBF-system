...
predict_with_glofas_and_rainfall <- function(all_days, rainfall, glofas, impact_df, rainfall_threshold, glofas_threshold, has_glofas) {
  rainfall <- rainfall %>%
  ...
  if ((nrow(rainfall) > 0) & has_glofas) {
    ...
    glofas %>%
      ...
      mutate(
        flood = replace_na(flood, FALSE),
        glofas_exceeds_threshold = dis >= glofas_threshold,
        flood_correct = flood & glofas_exceeds_threshold,
        next_exceeds_threshold = lag(glofas_exceeds_threshold),
        peak_start = !glofas_exceeds_threshold & next_exceeds_threshold
      ) %>%
      summarise(
        floods = sum(flood),
        floods_correct = sum(flood_correct),
