...
glofas_impact <- read.csv("data/ken_adm3.csv", stringsAsFactors = F) %>% 
  left_join(kenya_mapp,by='ADM2_EN')  %>% dplyr::select(ADM3_EN,ADM3_PCODE,station)%>% drop_na()%>%
  left_join(df_impact_raw[[2]] %>% 
              dplyr::mutate(flood = TRUE) , by = c("admin" = "admin")) %>%  #%>% unique()
  mutate(pcode = ADM3_PCODE) %>%dplyr::filter(!is.na(pcode))
