...
glofas_impact_uga <- read.csv("data/uga_affected_area_stations.csv", stringsAsFactors = F)

glofas_impact_uga <- glofas_impact_uga %>%
  ...
  left_join(df_impact_raw[[3]] %>% dplyr::mutate(flood = TRUE) %>% dplyr::select(-pcode), by = c("admin" = "admin")) %>%
...
glofas_raw_ <- glofas_raw %>%dplyr::filter(
  date >= min(glofas_impact_uga$date)-15,
  date <= max(glofas_impact_uga$date)+15) %>% filter(!is.na(dis_d))

day_range<-40
thresholds<-c('q5','q10','q20','q50','q75','q80','q95')


for (selected_district in glofas_impact_uga$admin %>% unique()) {
  df1<- glofas_impact_uga %>% filter(admin==selected_district)

for (selected_station in df1$station_name %>%  unique()) {
  df2<- glofas_raw_ %>% filter(station==selected_station)%>% left_join(df1 , by = "date") %>% distinct(date,.keep_all = TRUE)%>%
    filter(!is.na(dis_d))%>%
  ...
  for (elm in thresholds){
    ...
    df4<-data.frame(df3) %>% mutate(thr=thr,thr_stat=elm,district=selected_district,Glofas_st=selected_station) %>%
      dplyr::select(district,Glofas_st,thr_stat,thr,floods,floods_correct,floods_incorrect,protocol_triggered,
             triggered_in_vain,detection_ratio,false_alarm_ratio)
    if(nrow(df4[which(!is.na(df4$detection_ratio)),])>0){

      print(df4)
      write.table(df4, "output/Uganda_trigger_table_3days_leadtime.csv", sep = ",", col.names = !file.exists("output/Uganda_trigger_table_3days_leadtime.csv"), append = T, row.names = F)
  ...
  for (elm in thresholds){
    ...
    df4<-data.frame(df3) %>% mutate(thr=thr,thr_stat=elm,district=selected_district,Glofas_st=selected_station) %>%
      dplyr::select(district,Glofas_st,thr_stat,thr,floods,floods_correct,floods_incorrect,protocol_triggered,triggered_in_vain,detection_ratio,false_alarm_ratio)

    if(nrow(df4)>0){

      print(df4)
      write.table(df4, "output/Uganda_trigger_table_7days_leadtime.csv", sep = ",", col.names = !file.exists("output/Uganda_trigger_table_7days_leadtime.csv"), append = T, row.names = F)
