...
library(readr)
library(httr)
setwd("C:/Users/ATeklesadik/OneDrive - Rode Kruis/Documents/documents/IBF-system/trigger-model-development/flood/trigger-model/dashboard/kenya_dashboard")
...
glofas_mapping <- read.csv("data/kenya_affected_area_stations.csv",sep=";" ,stringsAsFactors = F)
...
Kenya_impact <- read_delim("data/Kenya_impact.csv", ";", escape_double = FALSE, trim_ws = TRUE)
...
kenya_admin1 <- kenya_admin1 %>%
  left_join(floods_per_wereda %>% dplyr::select(pcode, n_floods), by = c("ADM1_PCODE" = "pcode")) %>% dplyr::filter(!is.na(n_floods))
...
glofas_mapping <- glofas_mapping %>%
  left_join(df_impact_raw %>% dplyr::select(county, pcode) %>% unique(), by = c("County" = "county")) %>%
