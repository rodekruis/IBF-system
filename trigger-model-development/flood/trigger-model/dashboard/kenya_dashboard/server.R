server <- function(input, output) {
  ...
  selected_pcode <- reactiveVal("KE030")
  ...
  result_table <- reactive({
    ...
    predict_with_glofas(all_days, glofas(), impact_df(), input$glofas_threshold, isolate(has_glofas()), input$glofas_variable)
  })
  ...
  #output$rainfall_slider <- renderUI({sliderInput("rainfall_threshold", "Select Rainfall Threshold: ", min=0,max = round(max(isolate(rainfall())$rainfall, na.rm=T)),value=round(quantile(isolate(rainfall())$rainfall, 0.99, na.rm=T)))
  #})

  output$impact_map <- renderLeaflet({
    leaflet() %>%
      addProviderTiles(providers$OpenStreetMap) %>%
      addPolygons(data = kenya_admin1, label=~county, layerId=~pcode, col=~flood_palette(n_floods), fillOpacity=0.8, opacity = 1, weight=1.2)
  })

  observeEvent(input$impact_map_shape_click, {
    event <- input$impact_map_shape_click
    county_pcode <- event$id
    selected_pcode(county_pcode)
