...
glofas_st <- st_intersection(glofas_st, admin_zone) %>% 
  filter(id !='Na')  %>% mutate(station=id) %>% 
  select(station,name) #%>%   st_set_geometry(NULL)
...
impact_data1_w_ts <- impact_data %>% mutate(flood=as.numeric(flood),
                                              Deaths=as.numeric(Deaths),
                                              PplAffc=as.numeric(PplAffc),
                                            year = format(as.POSIXct(Date, format = "%d/%m/%Y"), format = "%Y"))
...
impact_data1_R<-impact_data1_w_ts %>% group_by(name) %>%  summarise(flood = sum(flood,na.rm=TRUE),
                                                                      geometry=st_union(geometry),
                                                                      Deaths = sum(Deaths,na.rm=TRUE),
                                                                      PplAffc = sum(PplAffc,na.rm=TRUE)) %>%ungroup()#%>%  st_set_geometry(NULL)
...
barplot(impact_data1_R$PplAffc, names = impact_data1_R$name,
        las = 3, cex.axis = 0.6, cex.names = 0.9, ylab = " Total Number of affected People", cex.lab = 0.9, space = 0,
        col = "#fc9272",main = "Most affected Districts\n by #of affected people",cex.main = .7)

barplot(impact_data1_R$Deaths, names = impact_data1_R$name,
        las = 2, cex.axis = 0.6, cex.names = 0.9, ylab = "Total Deaths", cex.lab = 0.9, space = 0,
        col = "#fc9272",main = "Most affected Regions\n by Deaths", cex.main = .7)

barplot(impact_data1_R$flood, names = impact_data1_R$name,
        las = 3, cex.axis = .6, cex.names = 0.9, ylab = "Total Number of Floods", cex.lab = 0.9, space = 0,
        col = "#fc9272",main = "Most affected Regions\n by Floods", cex.main = .7)
...
tmap_save(m1,width=1959, height=1791,dpi=400,
          filename = 'Affected_number_of_people_Uganda.png')
