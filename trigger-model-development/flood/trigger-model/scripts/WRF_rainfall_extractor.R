...
df <- data.frame(matrix(ncol=nrow(uga_admin)+1, nrow=0))
names(df) <- c("time",uga_admin$ADM2_EN)
date <- dmy("01-01-2000")
for (i in 0:4500){
  d <- date + i
  cat(sprintf("date: %s\n", d))
  r <- get_date_as_raster(nc, d)
  df[nrow(df)+1,2:ncol(df)] <- extract(r, uga_admin, fun = max)
  df[nrow(df),1] <- format(d, format = "%d-%m-%Y")
}

write.csv(df, "c:/Users/BOttow/Documents/IBF-system/trigger-model-development/flood/trigger-model/dashboard/IARP_trigger_dashboard/data/WRF_uganda_2000-2010.csv",
          row.names = F)
