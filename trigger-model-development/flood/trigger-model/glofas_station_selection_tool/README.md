...
#### Input files
* `Grid_layer_1.json`: .1컓 .1� degree grid cells of Uganda as explained in previous slide.
...
GloFas provides a gridded dataset at a 0.1째 resolution. The GloFAS Station Selection (GSS) tool uses an identical grid as an overlay. There are several ways to generate this grid. The following section contains a description of the approach we took to replicate the GloFAS grid using Qgis.
...
Using the example in figure 2, the middle is indicated by a yellow point. The (black) boundaries extent .05째 from that middle point. Creating a GloFAS grid requires starting (xmin, ymin) to start at a .1째 interval. 
...
Which brings up the menu shown below. For the grid of Uganda, Grid type was set to 'Rectangle (Polygon)', which makes the eventual grid clickable, Grid Extent was set to the bounding box of Uganda (xmin: 29.5, xmax: 35.2, ymin: -1.5, ymax: 4.3). Moreover, the horizontal and veritcal spacing were set to .1째 degrees and the Grid CRS was set to Default EPSG:4326 - WGS 84. Lastly, one can select the desired location (ie folder) for the shp file of the grid. 
