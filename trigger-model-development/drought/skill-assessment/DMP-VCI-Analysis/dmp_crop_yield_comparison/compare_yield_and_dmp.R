...
for (period in periods){
  ...
  dmp2 <- dmp  %>%
    left_join(dmp %>% filter(as.numeric(month) %in% period) %>% group_by(pcode,year) %>%
                summarise(dmp_maize_m = length(period) * 30 / 1000 * mean(dmp)),by=c("pcode","year"))
...
tmap_mode(mode = "view")
tm_shape(lhz2) + tm_polygons(col = "cor_maize_lzn", name='LZCODE',
                             ...
                             popup.vars = c("LZCODE","cor_maize_lzn"),
