...
class EmailClient(MailChimp):
    ...
    def sendNotification(self, emailContent): 
        title = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        campaign_data = self.populateCampaginData(LIST_ID, title, emailContent["subject"], "Flood")
    ...
    def populateCampaginData(self, list_id, title, subject, data, auto_tweet=False):
        campaign_data = {"settings": {"title": title,
