...
where country_code = 'EGY' and source = 'population' and lead_time='3-day'
group by date
order by date desc
limit 1
	) day3
...
select max(case when sum <> '--' and sum <> '0' then 1 else 0 end) as trigger
from "IBF-pipeline-output".calculated_affected
where country_code = 'EGY' and source = 'population' and lead_time='5-day'
group by date
order by date desc
limit 1
	) day5
...
select max(case when sum <> '--' and sum <> '0' then 1 else 0 end) as trigger
from "IBF-pipeline-output".calculated_affected
where country_code = 'EGY' and source = 'population' and lead_time='7-day'
group by date
order by date desc
limit 1
	) day7
...
SELECT t0.country_code
	...
	,fc_trigger
	,fc_rp
	,case when t0.trigger_level = 0 then null else fc/t0.trigger_level end as fc_perc
	,fc_prob as fc_prob
	,t0.geom
--into "IBF-pipeline-output".dashboard_forecast_per_station
FROM "IBF-static-input".dashboard_glofas_stations t0
LEFT JOIN "IBF-pipeline-output".triggers_rp_per_station t1
