...
class Forecast:
    def __init__(self, leadTimeLabel, leadTimeValue, country_code, model):
        ...
        if model == 'glofas':
            ...
            self.floodExtent = FloodExtent(leadTimeLabel, leadTimeValue, country_code,self.district_mapping,self.district_cols)
            self.exposure = Exposure(leadTimeLabel, country_code,self.district_mapping,self.district_cols)
  
        if model == 'rainfall':
            self.rainfall_triggers,self.rainfall_cols = self.db.downloadDataFromDb('IBF-static-input','EGY_rainfall_trigger_levels')
            self.rainfallData = RainfallData(leadTimeLabel, leadTimeValue, country_code, self.rainfall_triggers, self.rainfall_cols)
            self.exposure = Exposure(leadTimeLabel, country_code)
