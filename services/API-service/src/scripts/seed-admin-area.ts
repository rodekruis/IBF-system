...
export class SeedAdminArea implements InterfaceScript {
    ...
    public async run(): Promise<void> {
        ...
        const adminAreaRepository = this.connection.getRepository(
            AdminAreaEntity,
        );
        await Promise.all(
            countries.map(
                (country): Promise<void> => {
                    if (envCountries.includes(country.countryCodeISO3)) {
                        return this.seedCountryAdminAreas(
                            country,
                            adminAreaRepository,
                        );
                    } else {
                ...
                (area): Promise<void> => {
                    return adminAreaRepository
                        ...
                        .values({
                            ...
                            name:
                                area.properties[
                                    `ADM${country.defaultAdminLevel}_EN`
                                ],
                            placeCode:
                                area.properties[
                                    `ADM${country.defaultAdminLevel}_PCODE`
                                ],
                            placeCodeParent: area.properties[
                                `ADM${country.defaultAdminLevel - 1}_PCODE`
                            ]
                                ? area.properties[
                                      `ADM${country.defaultAdminLevel -
                                          1}_PCODE`
                                  ]
                                : null,
                            geom: (): string =>
