describe("Load the dashboard non triggered", () => {
    ...
    it("shows chat with blue about-trigger button", function () {
        ...
        cy.get("[data-test=chat-about-trigger]").should(
            "have.class",
            "ion-color-ibf-royal-blue"
        );
    });

    it("shows video guide in chat and opens popup", function () {
        cy.get("[data-test=video-guide-button]").click();
        // After click show video popover
        cy.get("[data-test=video-popover]");
    });

    it("shows aggregate values of 0", function () {
        cy.get("[data-test=aggregate-list]").each(() => {
            cy.get("[data-test=aggregate-number").contains("0");
        });
    });

    it("checks if layer matrix is hidden", function () {
        cy.get("[data-test=layers-control-menu]").should("not.be.visible");
    });

    it("show layer matrix after click", function () {
        cy.get("[data-test=layers-toggle]").click();
        cy.get("[data-test=layers-control-menu]").should("be.visible");
    });
});
