...
import { EmbeddedLocationStrategy } from 'src/app/services/embedded-location-strategy.service';
...
function isEmbeddedMode(): boolean {
  ...
  const hasEmbeddedHash =
    window.location.hash.includes('#IBFDashboard') ||
    window.location.hash.includes('IBFDashboard');
  const hasEmbeddedParam = window.location.search.includes('embedded=true');
  const hasEmbeddedDataAttribute =
    document.documentElement.hasAttribute('data-ibf-mode');
  const isEmbedded =
    hasCustomElement ||
    hasEmbeddedHash ||
    hasEmbeddedParam ||
...
(async () => {
  try {
    console.log(
      '🚀 Initializing IBF Dashboard Web Component (Angular Elements + zone.js)...',
    );
    ...
    const app = await createApplication({
      providers: [
            ...
            [
              { path: '', redirectTo: '/dashboard', pathMatch: 'full' },
              {
                ...
                loadComponent: () => {
                  ...
                  return import('src/app/test-component.component')
                    .then((m) => {
              ...
              },
              {
                ...
                loadChildren: () => {
                  ...
                  console.log(
                    '📂 Import path: src/app/pages/dashboard/dashboard.module',
                  );

                  return import('src/app/pages/dashboard/dashboard.module')
                    .then((m) => {
                      console.log(
        ...
        ...(isEmbedded
          ? [{ provide: LocationStrategy, useClass: EmbeddedLocationStrategy }]
          : []),
      ],
    });
    ...
    const originalAttributeChangedCallback =
      element.prototype.attributeChangedCallback;
    ...
    console.log(
      '✅ IBF Dashboard web component registered successfully with zone.js',
    );
