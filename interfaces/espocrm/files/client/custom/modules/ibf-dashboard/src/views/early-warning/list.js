...
define('custom:views/early-warning/list', ['views/list'], function (Dep) {
    ...
    return Dep.extend({
        ...
        applyUrlFiltersToCollection: function() {
            console.log('EarlyWarning List View: Applying URL filters to collection fetch parameters');
            
            const urlParams = this.getUrlParameters();
            if (!urlParams || (!urlParams.countryCodeISO3 && !urlParams.disasterType)) {
                console.log('EarlyWarning List View: No URL parameters found, using default collection behavior');
                return;
            }
            ...
            if (urlParams.countryCodeISO3) {
                ...
                console.log('EarlyWarning List View: Added countryCodeISO3 filter:', urlParams.countryCodeISO3);
            }
            ...
            if (urlParams.disasterType) {
                ...
                console.log('EarlyWarning List View: Added disasterType filter:', urlParams.disasterType.toLowerCase());
            }
            
            if (where.length > 0) {
                console.log('EarlyWarning List View: Setting collection where clauses:', where);
                ...
                this.collection.fetch = function(options) {
                    ...
                    console.log('EarlyWarning List View: API request data with filters:', options.data);
                ...
                console.log('EarlyWarning List View: Collection configured with URL filters - API will fetch filtered data');
            }
        },

        setupSearchPanelWithFilters: function() {
            console.log('EarlyWarning List View: Setting up search panel with URL filters');
            
            const urlParams = this.getUrlParameters();
            if (!urlParams || (!urlParams.countryCodeISO3 && !urlParams.disasterType)) {
                console.log('EarlyWarning List View: No URL parameters for search panel');
                return;
            }
            
            // Wait for the search panel to be created
            this.once('after:render', function() {
                console.log('EarlyWarning List View: Applying URL filters to search panel after render');
                this.applyFiltersToSearchPanel(urlParams);
            }.bind(this));
            
            // Also try with a delay as fallback
            setTimeout(function() {
                console.log('EarlyWarning List View: Applying URL filters to search panel with delay');
                this.applyFiltersToSearchPanel(urlParams);
            }.bind(this), 1000);
        },

        applyFiltersToSearchPanel: function(urlParams) {
            console.log('EarlyWarning List View: Applying filters to search panel:', urlParams);
            ...
            console.log('EarlyWarning List View: Found search manager, applying filters');
            ...
            if (urlParams.countryCodeISO3) {
                ...
                console.log('EarlyWarning List View: Added countryCodeISO3 to search panel:', urlParams.countryCodeISO3);
            }
            ...
            if (urlParams.disasterType) {
                ...
                console.log('EarlyWarning List View: Added disasterType to search panel:', urlParams.disasterType.toLowerCase());
            }
            
            // Apply the advanced search
            console.log('EarlyWarning List View: Setting advanced search data:', advanced);
            searchManager.set({
                advanced: advanced
            });
        ...
        showFilterStatus: function(params) {
            ...
            const filterInfo = [];
            if (params.countryCodeISO3) filterInfo.push(`Country: ${params.countryCodeISO3}`);
            ...
            if (filterInfo.length > 0) {
                ...
                this.once('after:render', function() {
                    console.log('EarlyWarning List View: Adding server-side filter status to DOM');
                    if (this.$el && this.$el.length) {
                        // Remove any existing filter status first
                        this.$el.find('.alert-info').remove();
                        this.$el.prepend(filterText);
                        console.log('EarlyWarning List View: Server-side filter status added to DOM');
                    }
                }.bind(this));
                ...
                setTimeout(function() {
                    if (this.$el && this.$el.length && this.$el.find('.alert-info').length === 0) {
                        this.$el.prepend(filterText);
                        console.log('EarlyWarning List View: Server-side filter status added via delayed method');
                    }
                }.bind(this), 500);
            }
        }
