define('modules/ibf-dashboard/views/early-action/list', [
  'views/list',
], function (Dep) {
  return Dep.extend({
    ...
    setup: function () {
      ...
      console.log(
        'EarlyAction List View - Setup called with search panel integration',
      );
    ...
    applyUrlFiltersToCollection: function () {
      ...
      if (!urlParams || (!urlParams.placeCode && !urlParams.disasterType)) {
        console.log(
          'No URL parameters found, using default collection behavior',
        );
        return;
      }
      ...
      if (where.length > 0) {
        ...
        console.log(
          'Collection configured with URL filters - API will fetch filtered data',
        );
      }
    },
    ...
    applyFiltersToSearchPanel: function (urlParams) {
      ...
      if (urlParams.disasterType) {
        ...
        console.log(
          'Added disasterType to search panel:',
          urlParams.disasterType.toLowerCase(),
        );
      }
    ...
    showFilterStatus: function (params) {
      ...
      if (filterInfo.length > 0) {
        const filterText = `<div class="alert alert-info" style="margin: 10px 0; padding: 10px; background-color: #d9edf7; border: 1px solid #bce8f1; color: #31708f; border-radius: 4px;">
                    ...
                    <a style="cursor:pointer; float: right; font-size: 24px; line-height: 24px; text-decoration: none;" href="/#EarlyAction" title="Remove filters">&times;</a>
                </div>`;
