...
# This script creates properly named Static Web Apps: 'ibf-dashboard-production' and 'ibf-dashboard-test'
# Both apps will be deployed to the same resource group: 'rg-ibf-svelte-dashboard'
...
function Deploy-Environment {
    ...
    Write-Host "Deployment token for $EnvName environment:" -ForegroundColor Cyan
...
Write-Host "Starting deployment of named environments..." -ForegroundColor Blue
$testSuccess = Deploy-Environment -EnvName "test"

if (-not $testSuccess) {
    Write-Host "Test environment deployment failed. Stopping." -ForegroundColor Red
    exit 1
}
...
Write-Host "3. Test both environments"
Write-Host "4. Update your EspoCRM integration to point to the production URL"
Write-Host ""
